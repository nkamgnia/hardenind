...
#Date: 09/05/2020
#AUthor: martial Nk
#Description: script for cis configuretion in centos7
...
chmod 600 /etc/rsyslog.confecho "ALL: ALL" >> /etc/hosts.deny			# CIS 3.4.3
chown root:root /etc/hosts.deny				# CIS 3.4.5
chmod 644 /etc/hosts.deny				# CIS 3.4.5
...
sed -i 's/^max_log_file_action.*$/max_log_file_action = keep_logs/'# CIS 5.1.2-5.1.7  
...
-a always,exit -F arch=b64 -S S mount -F auid>=1000 -F auid!=4294967295 -k mounts
...
-a always,exit -F path=/usr/bin/gpasswd -F perm=x -F auid>=1000 -F auid!=4294967295 -k privilegeda always,exit -F path=/usr/bin/mount -F perm=x -F auid>=1000 -F auid!=4294967295 -k privileged
